...
const {
  ...
  const ownerNumber ['255742233117']
...
async function loadSession() {
    try {
        ...
        const megaFileId = config.SESSION_ID.startsWith('ATHEEM~') 
            ? config.SESSION_ID.replace("ATHEEM~", "") 
            : config.SESSION_ID;
...
async function connectToWA() {
    console.log("[🔰] ATHEEM-MD Connecting to WhatsApp ⏳️...");
    ...
    conn.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            console.log('[🔰] ATHEEM MD connected to WhatsApp ✅');
            ...
            console.log('[🔰] Plugins installed successfully ✅');
                ...
                try {
                    ...
                    const upMessage = `╭─〔 *🤖 ATHEEM-MD BOT* 〕  
├─▸ *Ultra Super Fast Powerfull ⚠️*  
│     *World Best BOT ATHEEM-MD* 
╰─➤ *Your Smart WhatsApp Bot is Ready To use 🍁!*  

- *🖤 Thank You for Choosing ATHEEM-MD!* 
...
╰─🚀 *Powered By 𝚊𝚝𝚑𝚎𝚎𝚖 𝚑𝚊𝚖𝚒𝚜𝚒 — 𝗔𝗧𝗛𝗘𝗘𝗠 *`;
                    
                    await conn.sendMessage(conn.user.id, { 
                        image: { url: `https://i.postimg.cc/HWPVsbvT/IMG-20250730-WA0024.jpg` }, 
                        caption: upMessage 
                    });
...
BotActivityFilter(conn);	
 ...
 /// READ STATUS       
  ...
  const udp = botNumber.split('@')[0];
    const immumd = ('255742233117', '255655692031');
    
    const ownerFilev2 = JSON.parse(fs.readFileSync('./assets/sudo.json', 'utf-8'));  
    
    let isCreator = [udp, ...atheemmd, config.DEV + '@s.whatsapp.net', ...ownerFilev2]
...
if (isBanned) return; // Ignore banned users completely
 ...
 /* 
  app.get("/", (req, res) => {
  res.send("ATHEEM STARTED ✅");
  });
*/
  app.use(express.static(path.join(__dirname, 'lib')));

app.get('/', (req, res) => {
  res.redirect('/atheemtech.html');
});
